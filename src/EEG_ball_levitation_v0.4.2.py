# EEG ball levitation v0.5.0: Muse→EI spectral→Keras/TFLite→blower speed 0/50/100% via Photon 2, with EI spectral/raw test modes; 2025-11-15 13:30, Thomas Vikström
...
STABILITY_WINDOWS = 5  # (tests) number of last decisions to require stable target
SMOOTH_WINDOWS = 5  # (tests) number of last probabilities to average
USE_MEDIAN_SMOOTH = False  # (tests) True=median smoothing, False=mean smoothing
TARGET_THRESHOLD = 0.6

# For live blower mapping we use class history instead:
CLASS_HISTORY_WINDOWS = 4  # number of last predicted classes to majority-vote
...
def send_decision(ser, is_target: bool):
    ...
    Sends '1' or '0'. Live loop now uses send_blower_speed instead.
    """
...
def send_blower_speed(ser, blower_percent: int):
    """
    Send blower speed 0–100 (%) as ASCII to Photon 2.
    Photon side can map 0–100 to PWM duty cycle.
    """
    blower_percent = int(max(0, min(100, blower_percent)))
    line = f"{blower_percent}\n".encode("ascii")
    if ser is not None and ser.is_open:
        try:
            ser.write(line)
        except SerialException as e:
        ...
        print(f"[DEBUG] Blower command => {blower_percent} % (no serial)")


def map_class_to_blower_speed(pred_class: int) -> int:
    """
    Mapping requested:
    - 0 when sleep
    - 50 % when calm
...
def live_loop():
    ...
    Mapping:
      - sleep  (class 2) -> blower 0%
      - calm   (class 0) -> blower 50%
      - non_calm (class 1) -> blower 100%
    ...
    try:
        ...
        while True:
            ...
            # --- Blower mapping ---
            blower_speed = map_class_to_blower_speed(stable_class)
            ...
            print(
                ...
                f"blower={blower_speed} %"
            )

            # --- Send to Photon 2 ---
            send_blower_speed(ser, blower_speed)
